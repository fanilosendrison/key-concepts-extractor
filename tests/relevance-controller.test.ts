...
describe("RelevanceController", () => {
	...
	it("T-RC-02: flagged and confirmed → removed", async () => {
		const anthropic = createMockProvider("anthropic", [
			relevanceR1([{ target: "blockchain", reason: "unrelated" }]),
		]);
		const openai = createMockProvider("openai", [
			relevanceR2([{ target: "blockchain", verdict: "confirmed_off_topic", reason: "agree" }]),
		]);
		const result = await runRelevanceControl({
		...
		expect(result.report.removed.map((r) => r.target)).toContain("blockchain");
		expect(result.filteredList.find((c) => c.term === "blockchain")).toBeUndefined();
	});

	it("T-RC-03: flagged but defended → retained", async () => {
		const anthropic = createMockProvider("anthropic", [
			relevanceR1([{ target: "caching", reason: "seems off-topic" }]),
		]);
		const openai = createMockProvider("openai", [
			relevanceR2([{ target: "caching", verdict: "defended", reason: "affects consistency" }]),
		]);
		const result = await runRelevanceControl({
		...
		expect(result.report.review_rounds).toBe(2);
		expect(result.report.retained_after_dispute.map((r) => r.target)).toContain("caching");
	...
	it("T-RC-04: R2 confirms 1, defends 2 → 1 removed, 2 retained", async () => {
		...
		const openai = createMockProvider("openai", [
			relevanceR2([
				{ target: "x", verdict: "confirmed_off_topic", reason: "" },
				{ target: "y", verdict: "defended", reason: "relevant" },
				{ target: "z", verdict: "defended", reason: "relevant" },
			]),
		]);
		const result = await runRelevanceControl({
